...
% alike at this stage.
% The gamma terms interact D with the factors so each side carries its own betas,
% that is what makes the pooled alpha exactly equal the spread alpha (Prop 1).
...
\begin{tikzpicture}[font=\footnotesize, >=Stealth]
  ...
  \node[greybox, align=center] (reg) at (7.6,0)
       {one regression on the firm month data\\[2pt]
        $r_{it} \;=\; a \,+\, \bm{\alpha}\, D_{i,t-1} \,+\, \textstyle\sum_k (\beta_k + \gamma_k D_{i,t-1})\, f_{kt} \,+\, \varepsilon_{it}$};
...
\begin{tikzpicture}[font=\footnotesize, >=Stealth]
  ...
  \node[greybox, align=center] (reg) at (7.6,0)
       {the same regression, now with \hibl{firm fixed effects}\\[2pt]
        $r_{it} \;=\; \textcolor{accentred}{\bm{\mu_i}} \,+\, \textcolor{goetheblue}{\bm{\apred}}\, D_{i,t-1} \,+\, \textstyle\sum_k (\beta_k + \gamma_k D_{i,t-1})\, f_{kt} \,+\, \varepsilon_{it}$};
...
% Walk the arithmetic slowly, 115.3 bp per month times twelve is about 14 percent
% per year, risk adjusted, for as long as the firm lives, from deciles on a screen.
% 115.3 is the wedge B from Table 4. If probed, the residents' gap itself is
% G = -122.3 (p. 36), related through B = (1-lambda) G = 0.943 x 122.3, so using
% the wedge is the conservative choice.
...
\beq
\underbrace{115.3\ \text{bp per month}}_{\text{BM permanence wedge, Table 4}} \;\times\; 12
\;\;\approx\; \underbrace{\textcolor{accentred}{+13.8\%\ \text{per year}}}_{\text{risk adjusted, permanent}}
\eeq
\svs
...
\bit\itemsep2pt
  ...
  \item Point 1 \quad taken at face value the wedge is a striking anomaly of its own, about 14 percent per year for BM residents, characterize it and report residency portfolio returns
