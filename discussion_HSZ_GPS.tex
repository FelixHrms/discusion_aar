...
% Short firm lives, their joint panel has 12,366 firms and 1.55m firm months,
% about 125 months per firm (p. 47), so mu hat is a ten year average, not a type.
...
\bit\itemsep2pt
  ...
  \item Residency is \hibl{observable in real time}, so the claim implies a simple trading rule
\eit
\svs
\lead{What that rule earns at their own numbers}
\bit\itemsep2pt
  \item Book to market \quad long permanent growth, short permanent value residents \quad \hird{$+14.7\%$ p.a.}
  \item Momentum \quad long permanent winners, short permanent losers \quad \hird{$+38.2\%$ p.a.}
  \item Risk adjusted, permanent, read off a screen \quad {\color{softgrey}\footnotesize gaps of 122.3 and 318.1 bp per month, Table 4}
\eit
\svs
\lead{Either way the verdicts change}
\bit\itemsep2pt
  \item \higr{If real} \quad this is the paper's headline result, and subtracting priced compensation is wrong
  \item \hird{If not} \quad $\hat\mu_i$ is in sample luck over short firm lives, the reclassifications inherit the doubt
\eit
\svs
\lead{Asks}
\bit\itemsep2pt
  \item Report forward returns of portfolios sorted on \emph{past} residency
  \item Split firm lives, first half $\hat\mu_i$ against second half alpha. Types persist, luck does not
\eit
\end{frame}
