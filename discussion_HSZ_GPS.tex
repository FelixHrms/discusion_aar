...
% time, so it implies a strategy anyone can run.
% SAY the numbers slowly, 122 bp per month is about 15 percent per year, risk
% adjusted, forever, from a rule you read off a screen. Momentum residents 38.
% Then the fork, if real it is the biggest result in the paper, if not real the
% wedge is in sample history, luck and cohorts over short firm lives, and every
% Inflated or Compositional verdict inherits the doubt.
...
% The paper calls risk versus mispricing "logically downstream" (p. 9). Counter,
% it is upstream here, the meaning of every verdict depends on what B is.
\begin{frame}{Point 1: What is the wedge? Permanence taken at face value}
\lead{The paper reads the wedge as a fact about expected returns}
\bit\itemsep2pt
  \item Firms persistently in the top BM decile ``carry substantially lower unconditional \emph{expected} returns'' (p.~36), the residents' gap is $G$, and the wedge is $\hat B=(1-\lambda)\,G$
  \item Decile residency is \hibl{observable in real time}, so this implies a strategy, hold the residents with the favourable gap and short the others
\eit
\svs
\begin{center}
\stat{$+14.7\%$ p.a.}{book to market residents\\ $G=-122.3$ bp per month}\hspace{14pt}
\stat{$+38.2\%$ p.a.}{momentum residents\\ $G=+318.1$ bp per month}
\end{center}
\fignote{Residents' gaps $G$ and permanence $1-\lambda$ from Table 4 of the paper, market model, value weighted decile sorts, $\hat B=(1-\lambda)G$. For BM $1-\lambda=0.943$, so wedge and residents' gap nearly coincide. For momentum $1-\lambda=0.193$, persistent residents are rare. Annualization multiplies the monthly gap by twelve.}
\lead{Taking it at face value forces a choice, and both readings change the verdicts}
\bit\itemsep2pt
  \item \higr{If real} \quad an observable rule earns double digit risk adjusted returns forever. That is the paper's biggest result, not a correction term, and the sort alpha then contains priced compensation \citep{danieltitman1997} that should not be subtracted
  \item \hird{If not real} \quad $\hat\mu_i$ is \emph{realized} history over short firm lives (their panel averages 125 months per firm), luck and cohorts rather than a type, and every Inflated or Compositional verdict inherits this doubt
\eit
\svs
\lead{Asks, two cheap tests that separate the readings}
\bit\itemsep2pt
  \item Sort firms on \emph{past} residency and report the forward returns of residency portfolios
  \item Split firm lives, does first half $\hat\mu_i$ predict second half alpha? Types persist, luck does not
\eit
\end{frame}
...
\bit\itemsep2pt
  ...
  \item Point 1 \quad at face value the wedge is a double digit permanent alpha from an observable rule, test residency portfolios and identify $B$ before it re judges the zoo
